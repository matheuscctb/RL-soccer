...
class ContinuousActor(nn.Module):
    ...
    - Saída: Média e desvio padrão para ações [v_x, v_y, v_theta, kick_x]
    ...
    def __init__(self, obs_dim: int = 35, act_dim: int = 4, hidden_dim: int = 128):
        ...
        # Cabeça para média da ação (com ganho reduzido 0.01 para saídas iniciais próximas a zero)
        self.mu_head = init_layer(nn.Linear(hidden_dim, act_dim), std=0.01)

        # Parâmetro treinável para log_std (desvio padrão da política gaussiana)
